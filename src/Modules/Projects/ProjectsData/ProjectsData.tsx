    ...
    const fetchData = async () => {
      try {
        if (ProjectId && ProjectId !== "new-project") {
          ...
          const response = await privateAxiosInstance.get(
            PROJECTS_URLS.GET_PROJECT(cleanId)
          );
  ...
  const UpdateProject = async (data: TProjectData) => {
    ...
    try {
      ...
      const response = await privateAxiosInstance.put(
        PROJECTS_URLS.UPDATE_PROJECT(cleanId),
        projectData
      );
      ...
      toast.error(
        error instanceof Error ? error.message : "Error during Project update"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      <div className="table-container py-5">
        <Form
          className="mx-4 project-form text-start p-4"
          onSubmit={handleSubmit(isUpdate ? UpdateProject : onsubmit)}
        >
          <Form.Group>
            <Form.Label htmlFor="title" className="main-text-color ">
              Title
            </Form.Label>
          ...
          <Form.Group>
            <Form.Label htmlFor="Description" className="main-text-color ">
              Description
            </Form.Label>
            <InputGroup className="mb-3 ">
              <Form.Control
