...
const ViewProject: React.FC<ViewProjectProps> = ({ show, handleClose, showProjectId }) => {
  ...
  const getProjectById = async () => {
    ...
    setLoading(true);
    try {
      const response = await privateAxiosInstance.get(PROJECTS_URLS.GET_PROJECT(showProjectId));
  ...
  return (
    <Modal show={show} onHide={handleClose} size='lg' className="custom-modal d-flex justify-content-center align-items-center">
          ...
          <div>
            <div >
              ...
              <h5>
                ...
                <span className="fw-bold text-dark">{projectDataById.manager.userName}</span>
              </h5>
            </div>
            ...
            <div >
              ...
              <h5>
                ...
                <span className="fw-bold text-dark">{projectDataById.description}</span>
              </h5>
              <h5>
                <span className="text-muted fw-normal">Creation Date:</span>
                <span className="fw-bold text-dark">{new Date(projectDataById.creationDate).toLocaleString()}</span>
              </h5>
              <h5>
                <span className="text-muted fw-normal">Modification Date:</span>
                <span className="fw-bold text-dark">{new Date(projectDataById.modificationDate).toLocaleString()}</span>
              </h5>
            </div>
          </div>
        ) : (
