...
export default function TasksList() {
  ...
  const getTitleValue = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    getAllTasks(pageSize, 1, e.target.value, status);
  };
  ...
  const getAllTasks = async (
    ...
    try {
      ...
      const response = await privateAxiosInstance.get(
        TASKS_URLS.GET_ALL_MY_TASKS_FOR_MANAGER,
        {
          params: {
            pageSize: pageSize,
      ...
      setNumOfPagesArray(
        Array.from(
          { length: response?.data?.totalNumberOfPages || 0 },
          (_, index) => index + 1
        )
      );
    } catch (err) {
  ...
  const getStatusBadge = (status: string) => {
    if (status === "ToDo") {
      return (
        <span className="text-white rounded-5 px-3 py-1 bg-secondary">
          {status}
        </span>
      );
    } else if (status === "InProgress") {
      return (
        <span className="text-white rounded-5 px-3 py-1 bg-warning">
          {status}
        </span>
      );
    } else if (status === "Done") {
      return (
        <span className="text-white rounded-5 px-3 py-1 bg-success">
          {status}
        </span>
      );
    }
  ...
  //         placeholderOption.style.color = "#888";
  //         placeholderOption.style.fontSize = "0.8rem";
  //       } else {
  //         placeholderOption.style.color = ""; // Reset color
  //         placeholderOption.style.fontSize = ""; // Reset font size
  //       }
  //     }
  //   };
  ...
  return (
    <Container
      fluid
      className="container-bg p-0 m-0"
      style={{ height: "1000px" }}
    >
      <div className="w-100">
